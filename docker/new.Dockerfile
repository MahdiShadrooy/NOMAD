...
COPY ./skimpy /skimpy
COPY ./pytfa /pytfa
COPY ./NRAplus /NRAplus
COPY ./study-1-K_trpD9923 /study-1-K_trpD9923
RUN pip install -e /pytfa
RUN pip install -e /skimpy
RUN pip install -e /NRAplus
...
# ENTRYPOINT ["pip install -e ../../pytfa && \
# pip install -e ../../skimpy &&\
# pip install -e ../../NRAplus &&\
# 							$0 $*"]
# ENTRYPOINT ["/bin/bash", "-c", "pip install -e /nomad[ORACLE] && \
#                                $0 $*"]
