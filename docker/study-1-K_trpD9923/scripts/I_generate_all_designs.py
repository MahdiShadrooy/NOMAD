"""
This script takes in the 10 chosen kinetic models (K_trpD9923) and generates engineering strains for all of them
...
for this_model in kinetic_models:
    ...
    tols = nmodel.solver.configuration.tolerances
    if hasattr(tols, 'optimality'):
        tols.optimality = 1e-9
    if hasattr(tols, 'feasibility'):
        tols.feasibility = 1e-9
    if hasattr(tols, 'integrality'):
        tols.integrality = 1e-9
    
    print("Solver interface:", nmodel.solver.interface)

    # Check for parameters attribute before accessing
    if hasattr(nmodel.solver.problem, 'parameters'):
        if hasattr(nmodel.solver.problem.parameters, 'read'):
            if hasattr(nmodel.solver.problem.parameters.read, 'scale'):
                print("Parameter 'read.scale' exists. Setting to -1.")
                nmodel.solver.problem.parameters.read.scale = -1
            else:
                print("Parameter 'read.scale' does NOT exist.")
        else:
            print("'read' attribute does NOT exist under parameters.")
    else:
        print("'parameters' attribute does NOT exist for nmodel.solver.problem.")

    # emphasis.numerical
    if hasattr(nmodel.solver.problem, 'parameters'):
        if hasattr(nmodel.solver.problem.parameters, 'emphasis'):
            if hasattr(nmodel.solver.problem.parameters.emphasis, 'numerical'):
                print("Parameter 'emphasis.numerical' exists. Setting to 1.")
                nmodel.solver.problem.parameters.emphasis.numerical = 1
            else:
                print("Parameter 'emphasis.numerical' does NOT exist.")
        else:
            print("'emphasis' attribute does NOT exist under parameters.")
    else:
        print("'parameters' attribute does NOT exist for nmodel.solver.problem.")

    # presolve
    if hasattr(nmodel.solver.configuration, 'presolve'):
        print("Parameter 'presolve' exists. Setting to True.")
        nmodel.solver.configuration.presolve = True
    else:
        print("Parameter 'presolve' does NOT exist.")



    # print(nmodel.solver.configuration.tolerances.__dict__)
    ...
    if SIMULATE_IN_REACTOR:
        ...
        def reset_reactor():
            ...
            # Parameterize the rector and initialize with inoculum and medium data
        ...
        for this_design in designs:
            ...
            # Remove regulatoin of DDPA by Phe if DDPA is one of the targets
            if 'EU_DDPA' in this_design.keys():
                reactor.parameters['strain_1_ki_inhibitor1_DDPA'].value = 1e42
                reactor.parameters['strain_1_k_inhibition_IM_phe_L_c_DDPA'].value = 1e42
